...
public class VisualLeak extends Entity {

    private int animationTimer = 0, animationStage = 0;

    public VisualLeak(int x, int y, Game gameComponent) {
        super(0x5500000D, x, y, "visual leak", gameComponent);
            ...
            if(entity instanceof Player) {
                ...
                if(d <= 128 && this.getGameComponent().getDisplayComponent().getKeyboardInput().isPressed(KeyboardInput.KEY_USE)) {
                    if(this.getGameComponent().getWorld().getUtilityVisualCrystal() >= 1) {
                        this.getGameComponent().getWorld().setUtilityVisualCrystal( this.getGameComponent().getWorld().getUtilityVisualCrystal() - 1 );
                        this.getGameComponent().getWorld().setObjectiveStopVisualLeak( this.getGameComponent().getWorld().getObjectiveStopVisualLeak() + 1 );
    ...
    public void render(Graphics g) {
        g.drawImage(Texture.ANOMALY_VISUAL_LEAK.getImage().getSubimage(0, animationStage * 16, 16, 16), this.getX(), this.getY(), 64, 64, null);
    }
